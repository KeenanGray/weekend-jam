...
public class DisplacementFeaturePass : ScriptableRenderPass
{
     Material material;
    
    // The profiler tag that will show up in the frame debugger.
    const string ProfilerTag = "Displacement Pass";

    // We will store our pass settings in this variable.
    DisplacementFeature.PassSettings passSettings;
    ...
    static readonly int MagnitudeProperty = Shader.PropertyToID("_Magnitude");
    static readonly int ColorProperty = Shader.PropertyToID("_Color");
    static readonly int DisplacementMapProperty = Shader.PropertyToID("_DisplaceTex");


    // The constructor of the pass. Here you can set any material properties that do not need to be updated on a per-frame basis.
    public DisplacementFeaturePass(DisplacementFeature.PassSettings passSettings)
    {
        ...
        if(material == null) material = CoreUtils.CreateEngineMaterial("Shaders102/Displacement");
        
        // Set any material properties based on our pass settings. 
        material.SetTexture(DisplacementMapProperty, passSettings.displacement_map);
        material.SetFloat(MagnitudeProperty, passSettings.magnitude);
        material.SetColor(ColorProperty, passSettings.overlay_color);
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        using (new ProfilingScope(cmd, new ProfilingSampler(ProfilerTag)))
        {
            ...
            Blit(cmd, temporaryBuffer, colorBuffer, material); // shader pass 1
        }
